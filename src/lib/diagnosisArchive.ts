...
export function archiveCurrentDiagnosis(current: SavedAppStateV2, result: PathProgressResult, id: string, createdAt: string): SavedAppStateV2 {
  ...
  if (current.snapshots.some(previous => !previous.graduationPlan && snapshotFacts(previous) === snapshotFacts(snapshot))) return current;
