...
describe("browser storage acquisition failure", () => {
  ...
  it("keeps selections in React memory without claiming a durable save or reacquiring storage", async () => {
    ...
    await clickButton("내 정보 확인");
    await click('input[name="majorRole"][value="minor"]');
    await clickButton("내 정보 저장하고 계속");
    await click('input[name="entryIntent"][value="completed-courses"]');
    await clickButton("선택한 방법으로 시작하기");
    ...
    expect(document.querySelector('.track-history-basis')?.textContent).toContain("1과목");
    expect(document.querySelector(".planner-local-save--error")?.textContent).toContain("저장하지 못함");
