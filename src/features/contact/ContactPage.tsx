...
export function ContactPage({ headingRef }: {
  ...
  return <div className="dku-contact-page">
    ...
    <section className="dku-contact-page__official" aria-labelledby="contact-official">
      <div><span>신청·이수·발급 문의</span><h2 id="contact-official">{DEPARTMENT_SUPPORT.name}</h2><p>{DEPARTMENT_SUPPORT.room}</p></div>
      <address>
        ...
        <a href={`${DEPARTMENT_SUPPORT.emailUrl}?subject=${encodeURIComponent("트랙 삭제 문의")}`}><Mail size={17} aria-hidden="true" />트랙 삭제 문의 · {DEPARTMENT_SUPPORT.email}</a>
      </address>
    </section>
    ...
    <p><a href={DEPARTMENT_SUPPORT.curriculumUrl} target="_blank" rel="noopener noreferrer">학과 공개 교육과정 확인 <ExternalLink aria-hidden="true" size={16} /></a></p>
  </div>;
}
