...
export function OfficialResourcesView() {
  return <section aria-label="공식 근거와 확인 경로">
    <ul className="dku-resource-source-list">{sources.map((item, index) => <li key={item.href}><span>0{index + 1}</span><div><h2>{item.title}</h2><p>{item.text}</p></div><a href={item.href} target="_blank" rel="noopener noreferrer">원문 열기 ↗</a></li>)}</ul>
