    ...
    {rows.length ? (
      <div className="dku-resource-table-wrap">
        <table className="dku-resource-data-table dku-resource-timetable">
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={`${row.officialCourseCode}-${row.section}`}>
                <th scope="row">
                  <strong>{row.courseName}</strong>
                  <small>학사 과목코드 {row.officialCourseCode} · {row.section}분반</small>
                  <small>{row.grade}학년 · {row.projectCourseId ? `트랙 자료 코드 ${row.projectCourseId.toUpperCase()}` : "트랙 밖"}</small>
                </th>
                <td data-label="학점·교수">
                  {row.credits}학점
                  <small>교수 {row.instructor ?? "미표기"}</small>
                </td>
                <td data-label="요일·교시·시각">
                  {row.dayPeriods.map((period, index) => (
                    <small key={index}>{formatDayPeriod(period)}</small>
                  ))}
                </td>
                <td data-label="강의실·수업방식">
                  {row.delivery}
                ...
                <td data-label="확인 사항">
                  ...
                  <details>
                    <summary>조회 출처</summary>
                    <p>{TIMETABLE_QUERY_SCOPES[row.scope]}</p>
                    <p>원문: {row.scheduleRaw}</p>
                    <p>강좌 게시·수정일 미표기</p>
                  </details>
                </td>
              </tr>
      ...
      <div className="dku-resource-empty" role="status">
        <strong>조건에 맞는 분반이 없습니다.</strong>
        <p>조회 범위 안에서 검색된 결과입니다. 미개설·폐지를 뜻하지 않습니다.</p>
        <button type="button" onClick={() => { setQuery(""); setDay("all"); setDelivery("all"); setScope("all"); }}>
          검색 조건 초기화
        </button>
      </div>
    )}
