...
export function TrackSystemOverview() {
  return <section aria-label="다섯 트랙의 구성">
    ...
    <ol className="dku-resource-track-list">{tracks.map((track, index) => <li key={track.id} data-track-id={track.id}><span className="dku-resource-track-number">0{index + 1}</span><div><small>{track.kind}</small><h2>{track.name}</h2><p>{track.description}</p><div className="dku-resource-module-tags">{trackModules(track).map((id) => <span key={id}>{id} · {modules.find((item) => item.id === id)?.name}</span>)}</div><details><summary>현재 참고 계산 기준</summary>{track.rule.type === "major" ? <p>5개 모듈에서 각각 {track.rule.requiredCreditsPerModule}학점 · 합계 {track.rule.totalTrackCredits}학점</p> : <p>F/H/I 각각 3학점 이상, 합산 15학점 · M 8학점 · N+O 7학점 · 합계 30학점</p>}<p>제공된 최종안에 따른 참고 계산입니다. 개별 학생의 적용·공식 승인을 뜻하지 않습니다.</p></details></div></li>)}</ol>
  </section>;
}
