...
export function GuideApplication({ onStartDiagnosis }: { onStartDiagnosis: () => void }) {
  return <section className="guide-application" data-track-guide-section="application" aria-labelledby="track-guide-application-heading">
    ...
    <ol className="guide-application-steps" aria-label="트랙 신청 준비와 제출 순서">
      <li><span aria-hidden="true">01</span><div><h3>학교 공지에서 신청서 받기</h3><p>공지에 첨부된 HWP 신청서와 교육과정 자료를 확인하세요.</p><a className="guide-source-link" href={TRACK_APPLICATION_2026.noticeUrl} target="_blank" rel="noopener noreferrer">신청 공지·서식 확인<ExternalLink aria-hidden="true" /></a></div></li>
      <li><span aria-hidden="true">02</span><div><h3>신청할 트랙을 표시하고 서명하기</h3><p>성명·학과·학번·학년·휴대폰·이메일을 적고 원하는 트랙에 √ 표시하세요. 복수 선택이 가능합니다. 신청일과 서명도 작성하세요.</p></div></li>
      <li><span aria-hidden="true">03</span><div><h3>학과 사무실에 원본 제출하기</h3><p>작성한 신청서 원본을 <strong>{TRACK_APPLICATION_2026.submissionRoom}</strong>에 제출합니다.</p></div></li>
    </ol>
