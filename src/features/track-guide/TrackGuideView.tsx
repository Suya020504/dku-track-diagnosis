...
function OutcomesSection({ onContinue }: { onContinue: () => void }) {
  return <section className="guide-records" data-track-guide-section="outcomes" aria-labelledby="track-guide-outcomes-heading">
    ...
    <div className="guide-record-comparison">
      <article className="guide-degree"><span className="guide-eyebrow">현재 공식 명칭 · 2026 학칙 별표 2</span><GraduationCap aria-hidden="true" /><dl><div><dt>학위</dt><dd>경제학사</dd></div><div><dt>전공</dt><dd>식품자원경제학</dd></div></dl></article>
      <article className="guide-track-record"><span className="guide-eyebrow">운영 설명 · 2024 학과 공식 영상</span><h3>트랙은 어떤 분야를 공부했는지 나타내요</h3><p>트랙명은 별도의 학위명이 아닙니다. 학과에서 이수한 세부 학습 분야를 뜻해요.</p><p>학과 안내 영상에서는 트랙을 ‘세부 전공’의 성격으로 설명하며 이수 트랙명을 <strong>학위증·성적증명서</strong> 등에 기재하는 방식으로 안내했습니다.</p></article>
    </div>
...
function ApplicationSection({ onStartDiagnosis }: { onStartDiagnosis: () => void }) {
  return <section className="guide-application" data-track-guide-section="application" aria-labelledby="track-guide-application-heading">
    ...
    <div className="guide-application-context">
      <section aria-labelledby="application-known"><h3 id="application-known">내 이수 현황부터 확인하세요</h3><p>들은 과목과 남은 모듈을 정리해 두면 어떤 트랙을 준비할지 상담할 때 도움이 돼요.</p><p>2024년 안내 영상은 졸업 전 이수가 가능하면 2~4학년 학생도 신청할 수 있다고 설명합니다. 올해 신청 대상은 별도로 확인해야 해요.</p></section>
...
function VideosSection({ videoId, onVideoChange }: { videoId: OfficialTrackVideoId; onVideoChange: (videoId: OfficialTrackVideoId) => void }) {
  ...
  return <section className="guide-media" data-track-guide-section="videos" aria-labelledby="track-guide-videos-heading">
    ...
    <div className="guide-media-footnote"><p>영상은 제도 취지에 관한 2024년 설명입니다. 2026 현재 신청·인정 기준은 공개본과 학과에서 우선 확인해야 합니다.</p><SourceLink href={TRACK_LATE_ENTRY_VIDEO_URL}>중간 학년의 트랙 신청 · 2편 04:19</SourceLink><SourceLink href={TRACK_CERTIFICATE_VIDEO_URL}>트랙 이수 기록과 증명서 · 2편 06:50</SourceLink><SourceLink href={TRACK_DEGREE_VIDEO_URL}>학위명과 트랙명의 차이 · 3편 14:20</SourceLink><SourceLink href={TRACK_QA_VIDEO_URL}>2024 교육과정 개편 Q&A</SourceLink><SourceLink href={DEPARTMENT_YOUTUBE_URL}>학과 공식 YouTube 채널</SourceLink></div>
  </section>;
}
...
function OfficialSourceLedger() {
  const sources = [
    ...
    { title: "2026학년도 학사종합안내", body: "졸업 후 진로를 고려한 5개 트랙·15개 모듈, 트랙별 구성과 푸드바이오경제의 자연과학 연계 설명", href: OFFICIAL_CURRICULUM_SOURCE.url },
